...
module.exports = async (client, message, api, content) => {
    ...
    message.reply({content: content, embeds: [new EmbedBuilder().setImage(porno[0].data.url).setColor('Random')], components: [button]}).then(m => {
      ...
      collector.on('collect', async (int) => {
        ...
        if (int.customId === 'próximo') {
          ...
          if (porno.length - 1 === index) {
            ...
            porno.push(req)
            m.edit({embeds: [new EmbedBuilder().setImage(req.data.url).setColor('Random')], components: [button]})
          } else {
            index++
          ...
          m.edit({embeds: [new EmbedBuilder().setImage(porno[index].data.url).setColor('Random')], components: [button]})
        } else {
          m.delete() 
